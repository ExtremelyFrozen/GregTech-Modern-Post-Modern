...
public class ComputationNetworkManager {
    ...
    public int getNetWorkMaxCWUt(ComputationPortTrait port) {
        for (ComputationNetwork network : networks) {
    ...
    public int getNetWorkUsedCWUt(ComputationPortTrait port) {
        for (ComputationNetwork network : networks) {
            if (network.contains(port)) {
    ...
    public int getNetWorkAvailableCWUt(ComputationPortTrait port) {
        for (ComputationNetwork network : networks) {
            if (network.contains(port)) {
    ...
    private static class ComputationNetwork {
        ...
        private void replaceNodes(List<ComputationPortTrait> newNodes) {
            ...
            for (ComputationPortTrait node : newNodes) {
        ...
        private void notifyChangedConsumers(ComputationSolver.Result result) {
            ...
            Set<ComputationConsumer> changedConsumers = new HashSet<>(lastAllocations.keySet());
            changedConsumers.addAll(allocations.keySet());
            for (ComputationConsumer consumer : changedConsumers) {
