...
public class TransformerBlockProvider extends MachineInfoProvider<TransformerMachine, CompoundTag> {
    ...
    protected CompoundTag write(TransformerMachine transformer) {
        ...
        data.putInt("side", transformer.getFrontFacing().get3DDataValue());
        data.putBoolean("transformUp", transformer.isTransformUp());
        data.putInt("baseAmp", transformer.getBaseAmp());
        data.putInt("baseVoltage", transformer.getTier());
        return data;
    }
    ...
    protected void addTooltip(CompoundTag data, ITooltip tooltip, Player player, BlockAccessor block,
        ...
        if (transformUp) {
            tooltip.add(Component.translatable("gtpm.top.transform_up",
                    (GTValues.VNF[voltage] + " §r(" + amp * 4 + "A) -> " + GTValues.VNF[voltage + 1] + " §r(" +
                            amp +
                            "A)")));
        } else {
            tooltip.add(Component.translatable("gtpm.top.transform_down",
                    (GTValues.VNF[voltage + 1] + " §r(" + amp + "A) -> " + GTValues.VNF[voltage] + " §r(" +
                            amp * 4 +
                            "A)")));
        }

        if (block.getHitResult().getDirection() == Direction.from3DDataValue(data.getInt("side"))) {
            tooltip.add(
                    Component.translatable(
                            (transformUp ? "gtpm.top.transform_output" : "gtpm.top.transform_input"),
                            (GTValues.VNF[voltage + 1] + " §r(" + amp + "A)")));
        } else {
            tooltip.add(
                    Component.translatable(
                            (transformUp ? "gtpm.top.transform_input" : "gtpm.top.transform_output"),
