...
public class MultiblockStructureProvider extends MachineInfoProvider<MultiblockControllerMachine, ByteTag> {
    ...
    protected void addTooltip(ByteTag data, ITooltip tooltip, Player player, BlockAccessor block,
        ...
        if (data.getAsByte() == 0) {
            tooltip.add(Component.translatable("gtpm.top.invalid_structure").withStyle(ChatFormatting.RED));
        } else {
            tooltip.add(Component.translatable("gtpm.top.valid_structure").withStyle(ChatFormatting.GREEN));
        }
    }
}
