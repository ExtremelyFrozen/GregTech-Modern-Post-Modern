...
public class MultiblockStructureProvider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor> {
    ...
    public void appendTooltip(ITooltip iTooltip, BlockAccessor blockAccessor, IPluginConfig iPluginConfig) {
        if (blockAccessor.getServerData().contains("hasError")) {
            ...
            if (hasError) {
                iTooltip.add(Component.translatable("gtpm.top.invalid_structure").withStyle(ChatFormatting.RED));
            } else {
                iTooltip.add(Component.translatable("gtpm.top.valid_structure").withStyle(ChatFormatting.GREEN));
            }
        }
    }
