...
/**
 * Opening-scoped amount projection shared by the ME fluid editor and wheel controls.
 */
@ParametersAreNonnullByDefault
@MethodsReturnNonnullByDefault
final class AEFluidConfigAmountProjection {
    ...
    AEFluidConfigAmountProjection(Supplier<AEFluidConfigSnapshot> snapshotSupplier,
    ...
    boolean requestAmount(int slot, int amount) {
        if (amount <= 0) {
            throw new IllegalArgumentException("ME fluid configuration amount must be positive: " + amount);
        }
        ...
        actionSender.accept(holder, MEFluidConfigActions.createSetAmountAction(
    ...
    private @Nullable AuthoritativeAmount authoritativeAmount(int slot) {
        ...
        if (!(config.what() instanceof AEFluidKey key)) {
            throw new IllegalStateException("ME fluid amount projection received a non-fluid configuration key.");
        }
        return new AuthoritativeAmount(key, Math.toIntExact(config.amount()));
    }

    /** Latest unconfirmed amount requested for one exact slot and fluid identity. */
    private record PendingAmount(AEFluidKey expectedKey, int desiredAmount) {}

    /** Authoritative slot state used as the base for an optional pending request. */
    private record AuthoritativeAmount(AEFluidKey key, int amount) {}
}
