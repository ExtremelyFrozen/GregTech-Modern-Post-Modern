...
The new metadata section is `"gtpm"` and no longer uses LDLib metadata at runtime.
A few regexes for converting old LDLib-style MCMeta files are as follows, run them in order:

1. Match: `"ldlib":([\s\{]*)"connection"(:\s".*?")(?:,\s.*)?`  
   Replace with: `"gtpm":$1"connection_texture"$2`
2. Match: `\s*\},\s*"shimmer":\s*\{(\s*"bloom":.*)`  
   Replace with: `,$1` 
3. Match: `(\{\s*)"shimmer":(\s*\{\s*"bloom":.*)`  
   Replace with: `$1"gtpm":$2`
...
- `ICopyable::copyConfig` now mutates the passed `CompoundTag` directly instead of returning one.
